...
public class ReviewService : IReviewService
{
    ...
    public async Task<PaginatedResponse<ReviewDto>> GetPageAsync(int pageNumber, int pageSize)
    {
        ...
        var totalReviews = await _reviewRepository.FindAsync(r => r.CustomerId == customerId);
    ...
    {
        var customerId = context.GetCustomerId();
        var reviews = await _reviewRepository.FindAsync(r => r.AutoServiceId == autoServiceId && r.CustomerId == customerId);
        return _mapper.Map<IEnumerable<ReviewDto>>(reviews);
    }
    ...
    private async Task UpdateAutoServiceRatingAsync(string autoServiceId)
    {
        ...
        var reviews = await _reviewRepository.FindAsync
            (r => r.AutoServiceId == autoServiceId && r.CustomerId == customerId);
